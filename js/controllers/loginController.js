...
assignmentApp.controller("LoginController", [
  "$scope",
  "dataService",
  "applicationData",
  "$location",
      ...
      dataService.login($scope.login).then(
        function(response) {
          ...
          if (response.status != 401) {
            if (response.data.user.initialised == false) {
              // first time user has logged on
              resetPasswordModal.style.display = "block";
              $scope.message =
                "This is the first time you are logging in. Please reset your password.";
            } else {
              // user already initialised
              $scope.errorMessage = "";
              $scope.message = "";
